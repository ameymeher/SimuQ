...
def clean_as(boxes, edges, backend, with_measure=True,noise_factor=0, circuit_length=0):
    n, link = get_n_link(backend)
    circ = QuantumCircuit(circuit_length)
    DG = nx.DiGraph()
...
def transpile(backend, alignment, sol_gvars, boxes, edges, use_pulse=True, with_measure=True,noise_factor=0,circuit_length=0):
    if use_pulse:
        pm = get_pm(backend)
        # return clean_as(boxes, edges, backend)
        return pm.run(clean_as(boxes, edges, backend, with_measure,noise_factor,circuit_length))
    else:
        return clean_as(boxes, edges, backend, with_measure,circuit_length=circuit_length)
