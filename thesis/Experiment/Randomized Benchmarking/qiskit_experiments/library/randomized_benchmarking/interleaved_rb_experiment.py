...
class InterleavedRB(StandardRB):
    ...
    def circuits(self) -> List[QuantumCircuit]:
        ...
        if self.pm is not None:
            return list(map(lambda x: self.pm.run(x.decompose()),list(itertools.chain.from_iterable(zip(reference_circuits, interleaved_circuits)))))
...
class InterleavedRBCustom(StandardRB):
    """An experiment to characterize the error rate of a specific gate on a device.
